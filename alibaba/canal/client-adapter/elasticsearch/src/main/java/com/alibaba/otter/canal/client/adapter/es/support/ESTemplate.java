...
public class ESTemplate {
     ...
     * @return 返回 id 值
     */
    public Object getESDataFromDmlData(ESMapping mapping, Dml dml, Map<String, Object> dmlData, Map<String, Object> dmlOld,
                                       Map<String, Object> esFieldData) {
        SchemaItem schemaItem = mapping.getSchemaItem();
        Iterator<SchemaItem.TableItem> iterator = schemaItem.getAliasTableItems().values().iterator();
        String alias = "";
        while (iterator.hasNext()) {
            SchemaItem.TableItem tableItem = iterator.next();
            if(tableItem.getTableName().equalsIgnoreCase(dml.getTable())){
                alias = tableItem.getAlias();
                break;
            }
        }
        String idFieldName = mapping.get_id() == null ? mapping.getPk() : mapping.get_id();
        ...
        for (FieldItem fieldItem : schemaItem.getSelectFields().values()) {
            ...
            if (dmlOld.containsKey(columnName) && alias.equalsIgnoreCase(fieldItem.getOwner()) && !mapping.getSkips().contains(fieldItem.getFieldName())) {
                esFieldData.put(Util.cleanColumn(fieldItem.getFieldName()),
